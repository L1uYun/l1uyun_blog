...
---
# 前言
之前打了一个linux域的靶机,在巨魔的wp站里看见了红日靶场系列,第一次打windows域的靶场,边看wp边打了
# 红日靶场1-win域
## 环境搭建
...
尝试在外面连接,连不上去
```bash
➜  workspace  mysql -h 192.168.249.79 -u root -p
Enter password:
```
...
# l1uyun
第一次打win域的机器,过段时间再打一下这台机器,这次完全是在学习了
